...
export function play_game(game_choices, player_choice) {
    ...
    pchoice_lower = player_choice.toLowerCase();
    oppchoice_lower = opponent_choice.toLowerCase();

    if (!game_choices.includes(pchoice_lower)) { // test if out of range
        console.error(pchoice_lower + ' is out of range.');

        if(game_choices == rpsls_choices) {
            console.log(
                `
            ...
            process.exit(1);
        } else if (game_choices == rps_choices) {
            console.log(
                `
